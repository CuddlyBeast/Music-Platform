...
document.addEventListener('DOMContentLoaded', async function() {
    try {
        ...
        const totalResponse = await fetch(`${BASE_URL}chill/newCountry?q=upbeat%20party%20exciting%20fast%20happy%20dance%20country&type=playlist`);
...
async function fetchPlaylistsByPage(page, limit) {
    const response = await fetch(`${BASE_URL}chill/newCountryLimit?q=upbeat%20party%20exciting%20fast%20happy%20dance%20country&type=playlist&page=${page}&limit=${limit}`);
    if (!response.ok) {
